...
Set `SAFEROUTE_ENABLE_PREVIEW_MODE=true` only in non-production simulator/dev runs to open the signed-in map home with local SafeRoute fixtures. `SAFEROUTE_PREVIEW_INITIAL_SCREEN=operations` opens the view-only Operations surface, and `SAFEROUTE_PREVIEW_INITIAL_SCREEN=routes` opens Saved routes, so authenticated supporting-flow UI smoke tests do not need real LunarChain credentials. Preview mode is ignored when `SAFEROUTE_APP_ENV=production`.
