...
describe('SafeRoute Maestro npm runner', () => {
  ...
  it('retries Maestro test flows once by default for transient local XCUITest drops', async () => {
    ...
    assert.equal(createMaestroProcessEnv({}).MAESTRO_DRIVER_STARTUP_TIMEOUT, '120000');
    assert.equal(
      createMaestroProcessEnv({ MAESTRO_DRIVER_STARTUP_TIMEOUT: '180000' }).MAESTRO_DRIVER_STARTUP_TIMEOUT,
      '180000'
    );
    ...
    assert.deepEqual(spawnedTimeouts, ['120000', '120000']);
