...
Set `SAFEROUTE_ENABLE_PREVIEW_MODE=true` only in non-production simulator/dev runs to open the signed-in map home with local SafeRoute fixtures. `SAFEROUTE_PREVIEW_INITIAL_SCREEN=operations` opens the view-only Operations surface, `SAFEROUTE_PREVIEW_INITIAL_SCREEN=routes` opens Saved routes, `SAFEROUTE_PREVIEW_INITIAL_SCREEN=login-code` opens the two-factor code UI, and `SAFEROUTE_PREVIEW_INITIAL_SCREEN=session-expired` opens the sign-in recovery notice, so supporting-flow UI smoke tests do not need real LunarChain credentials or OTP. Preview mode is ignored when `SAFEROUTE_APP_ENV=production`.
