...
- Release environment: production iOS config requires `SAFEROUTE_APP_ENV=production`, an explicit hosted HTTPS `SAFEROUTE_PROD_API_URL` that is not localhost or loopback, and an explicit `SAFEROUTE_IOS_BUILD_NUMBER`.
...
7. `SAFEROUTE_APP_ENV=production`, hosted HTTPS `SAFEROUTE_PROD_API_URL` (not localhost or loopback), and the exact `SAFEROUTE_IOS_BUILD_NUMBER` used by the signed artifact.
...
The pure helper at `src/config/firebaseDistributionReadiness.ts` maps local distribution inputs into explicit blockers and produces a Firebase upload command only when every required input is present. It also checks the production iOS release environment (`SAFEROUTE_APP_ENV=production`, valid hosted HTTPS API URL that is not localhost/loopback, and valid build number). It is covered by `test/firebaseDistributionReadiness.test.ts` and is intended for no-build release planning, not for generating artifacts.
...
- Confirm the signed artifact was produced with production app config, an explicit hosted HTTPS `SAFEROUTE_PROD_API_URL` that is not localhost or loopback, and the recorded build number.
